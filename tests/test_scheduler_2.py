...
from scripts.scheduler_2 import BatteryOptimizationScheduler, PyomoOptimizationModelBuilder, GLPKOptimizationSolver  # noqa: E402
...
@pytest.fixture
def sample_battery():
    return Battery(capacity_mwh=1, charge_efficiency=0.9, discharge_efficiency=0.9, initial_soc=0.5, initial_soh=1.0)
...
def sample_scheduler(sample_battery, sample_prices):
    ...
    return BatteryOptimizationScheduler(sample_battery, sample_prices, model_builder, solver)
...
def test_create_schedule(sample_scheduler: BatteryOptimizationScheduler, sample_prices):
    ...
    assert len(schedule_df) == len(sample_prices)  # Ensure schedule length matches number of intervals
...
def test_handle_zero_prices(sample_battery):
    ...
    model_builder = PyomoOptimizationModelBuilder()
    solver = GLPKOptimizationSolver()
    scheduler = BatteryOptimizationScheduler(sample_battery, prices, model_builder, solver)
...
def test_energy_cycled_limit(sample_scheduler: BatteryOptimizationScheduler):
    ...
    max_allowed_cycled = sample_scheduler.battery.capacity_mwh * 2 * 5  # Example calculation
