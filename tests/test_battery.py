...
def test_charge_within_capacity():
    battery = Battery(capacity_mwh=1.0, initial_soc=0.5)
    battery.charge(energy_mwh=0.4, duration_hours=0.5)
...
def test_discharge_within_capacity():
    battery = Battery(capacity_mwh=1.0, initial_soc=0.5)
    battery.discharge(energy_mwh=0.2, duration_hours=0.5)
...
def test_soh_degradation():
    ...
    battery.charge(energy_mwh=0.5, duration_hours=0.5)
    battery.discharge(energy_mwh=0.5, duration_hours=0.5)
...
def test_charge_rate_limit():
    battery = Battery(capacity_mwh=1.0, initial_soc=0.0, max_charge_rate_mw=0.2)
    battery.charge(energy_mwh=0.5, duration_hours=0.5)
...
def test_discharge_rate_limit():
    battery = Battery(capacity_mwh=1.0, initial_soc=1.0, max_discharge_rate_mw=0.2)
    battery.discharge(energy_mwh=0.5, duration_hours=0.5)
...
def test_full_charge_discharge_cycle():
    ...
    battery.charge(energy_mwh=1.0, duration_hours=1)
    assert battery.soc == 0.9
    battery.discharge(energy_mwh=1.0, duration_hours=1)
    assert battery.soc == 0.0
...
def test_partial_charge_discharge():
    ...
    battery.charge(energy_mwh=0.25, duration_hours=0.5)
    assert np.isclose(battery.soc, 0.475)
    battery.discharge(energy_mwh=0.15, duration_hours=0.5)
...
def test_edge_case_soc_values():
    battery = Battery(capacity_mwh=1.0, initial_soc=0)
    battery.discharge(energy_mwh=0.1, duration_hours=0.5)
    assert battery.soc == 0.0

    battery = Battery(capacity_mwh=1.0, initial_soc=1.0)
    battery.charge(energy_mwh=0.1, duration_hours=0.5)
    assert battery.soc == 1.0
